...
print(np.amin(VpVsRho[:,:,:,1]), np.amax(VpVsRho[:,:,:,1]))
print(np.amin(VpVsRho[:,:,:,0]), np.amax(VpVsRho[:,:,:,0]))
...
####Creating the netcdf file
from netCDF4 import Dataset
rootgrp = Dataset("output_files/rho_col_average.nc", "w", format="NETCDF4")
...
####Creating the netcdf file
from netCDF4 import Dataset
rootgrp = Dataset("output_files/plastCo.nc", "w", format="NETCDF4")
...
rootgrp = Dataset("output_files/Ridgecrest_RhoMuLambda_cvms1000.nc", "w", format="NETCDF4")
...
print(np.amin(mu), np.amax(mu))
print(np.amin(lam), np.amax(lam))
print(np.amin(rho_col_average), np.amax(rho_col_average))
